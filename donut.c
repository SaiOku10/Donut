      ...
      memset(z, 0, 7040); //Fill z with 0
        for (j = 0; 6.28 > j; j += 0.07) { 
            for (i = 0; 6.28 > i; i += 0.02) {
                ...
                int x = 40 + 30 * mess * (cosi * cosj2 * cosB - t * sinB), //x coordinate calculation
		  y = 12 + 15 * mess * (cosi * cosj2 * sinB + t * cosB), //y coordinate calculation
		  ...
		  N = 8 * ((sinj * sinA - sini * cosj * cosA) * cosB - sini * cosj * sinA - sinj * cosA - cosi * cosj * sinB);//Calculate character index N
                if (22 > y && y > 0 && x > 0 && 80 > x && mess > z[o]) {// Check if the point is within visible range and brighter than previous
                    z[o] = mess; // Update brightness in 'z' array
